...
def get_temps() -> list["SensorGroup"]:
    ...
    return (
        [
            ...
            for group_name, sensor_readings in temps.items()
        ]
        if temps
        else []
    )
...
def get_cpu_temp(temps: list["SensorGroup"]) -> shwtemp | None:
    for group in temps:
        if group.label == Component.CPU or (
            group.label == Component.UNKNOWN and "cpu" in group.raw_name.lower()
        ):
...
class Flex(commands.Cog):
    ...
    async def flex(self, ctx: commands.Context, show_all_sensors: str = ""):
        ...
        embed.add_field(
            name="Memory Usage",
            value=f"{round(used_memory)} GB / {round(total_memory, 2)} GB ({round((used_memory / total_memory) * 100)}%)",  # noqa: E501
            inline=True,
        )
        embed.add_field(
            name="Swap Usage",
            value=f"{round(used_swap, 2)} GB / {round(total_swap, 2)} GB ({round((used_swap / total_swap) * 100)}%)",  # noqa: E501
            inline=True,
        )
        ...
        embed.add_field(name="OS", value=f"{os_name} {os_ver}", inline=True)
        ...
        if temps and show_all_sensors_bool:
            temps_embed = discord.Embed(title="System Temperatures", color=discord.Color.blue())
