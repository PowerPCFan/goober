...
class FinlandLarper(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.settings: FinlandLarperSettings = settings_manager.get_plugin_settings("finlandlarper", {"chance": 1})  # type: ignore
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.regex.search(message.content):
            try:
                ...
                logger.debug(f"[FinlandLarper] Message matched regex, rolled {randint} (1 in {self.chance} chance)")
                if randint == 1:
