...
class Eightball(commands.Cog):
    ...
    async def eightball(self, ctx: commands.Context, *, question: str = "") -> None:
        ...
        choice = random.choice(self.responses)  # noqa: S311
        ...
        await asyncio.to_thread(
            renderer.generate_eightball_gif,
            choice,
            output_path,
            question,
            ctx.author.name,
...
class EightballRenderer:
    ...
    def render_answer_overlay(self, text: str, opacity: int) -> Image.Image | None:  # noqa: C901, PLR0915
        ...
        triangle = [
            ...
            (
                self.center - self.triangle_width / 2,
                self.center + self.triangle_height / 2 + triangle_offset,
            ),
            (
                self.center + self.triangle_width / 2,
                self.center + self.triangle_height / 2 + triangle_offset,
    ...
    def _truncate_text(
        ...
        trimmed = text
        min_len = 3
        while len(trimmed) > min_len and draw.textlength(trimmed + "...", font=font) > max_width:
            trimmed = trimmed[:-1]
    ...
    def generate_eightball_gif(
        ...
        base_badge = self.render_ball_base(show_badge=True)
        base_answer = self.render_ball_base(show_badge=False)
        ...
        for opacity in range(0, 256, 8):
            ...
            if not answer_overlay:
                exc = "Failed to render answer overlay"
